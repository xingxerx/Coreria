...
if %errorlevel% neq 0 (
    ...
    g++ -std=c++11 main_dream_weaver_complete.cpp GameObject3D.cpp -o dream_weaver_complete.exe
)
...
echo Building complete game...
cd /d "%~dp0src\dream_weavers_heart"
cl /EHsc /std:c++17 main_dream_weaver_complete.cpp GameObject3D.cpp /Fe:dream_weaver_complete.exe >nul 2>&1
if %errorlevel% neq 0 (
    echo ❌ C++17 build failed! Trying without version flag...
    cl /EHsc main_dream_weaver_complete.cpp GameObject3D.cpp /Fe:dream_weaver_complete.exe >nul 2>&1
)

echo Building RPG characters demo...
cd /d "%~dp0src\epoch_of_elria"
cl /EHsc main_rpg_characters.cpp /I../dream_weavers_heart /Fe:rpg_characters.exe >nul 2>&1
